...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 25 20:54:44 2019
...
class Stocker():
    ...
    def __init__(self, ticker, exchange='WIKI'):
        ...
        #get dates we want to look at
        today = datetime.datetime.today().strftime('%Y-%m-%d')
        month_ago = datetime.datetime.utcnow()-datetime.timedelta(days = 5*365)
        
        # Symbol is used for labeling plots
        self.symbol = ticker
        
        # Use Personal Api Key
        quandl.ApiConfig.api_key = '43mHMAHQTtSZ_3WRVhDr'

        # Retrieval the financial data
        try:
            '''
            #stock = quandl.get('%s/%s' % (exchange, ticker))
            stockLogo = 'EOD/' + ticker
            stock = quandl.get(stockLogo, start_date=month_ago, end_date=today)
            '''
            stock = stockMarketData.stockchart(ticker)
        ...
        #Convert date in df from string to datetime
        format_str = '%Y-%m-%d' # The format
        stock.loc[:, 'ds'] = stock.loc[:,'ds'].map(lambda x: datetime.datetime.strptime(x, format_str))
        #stock.loc[:, 'ds'] = stock.loc[:,'ds'].map(lambda x: datetime.strptime(x, "%Y-%m-%d"))
        stock['date'] = stock['ds']
        ...
        stock.to_csv("stockdata.csv", index=False)
    ...
    def predict_future(self, days=7):
        ...
        future.to_csv("predictions.csv", index=False)
        '''
        # Print out the dates
        print('\nPredicted Increase: \n')
